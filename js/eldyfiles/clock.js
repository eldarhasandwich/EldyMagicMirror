...
const get12HrTime = (time) => {
	const [h, m] = [time.getHours(), time.getMinutes()].map(padTwoZeros);
	const hourModulo = h % 12;

	return `${hourModulo === 0 ? 12 : hourModulo}:${m}${h >= 12 ? "pm" : "am"}`;
};

const getDeltaTime = (targetTime, currentTime) => {
...
const clockUpdate = () => {
	...
	const currentTimeString12hr = get12HrTime(currentTime);
